...
class ProductionInput extends Model
{
    ...
    public function getActivityDescription(string $action): string
    {
        ...
        return ucfirst(strtolower($action)) . " Input ({$itemName}) in Batch ({$batchNumber})";
    }
    ...
    public function getActivityProperties(string $action): array
    {
        ...
        if ($action === 'UPDATE') {
            ...
            // If usage increases, stock decreases more (negative)
            $stockChange = - ($newQuantity - $originalQuantity);
        } elseif ($action === 'CREATE') {
            // New input usage reduces stock
            $stockChange = -$this->quantity;
        } elseif ($action === 'DELETE') {
            // Deleting input usage returns stock
            $stockChange = $this->quantity;
        }
